...
def generate_clf(clf_name, transforms, **kwargs):
    """
    Generate the *CLF* file for given transforms and given id.
    ...
    group_transform.write(
        "Academy/ASC Common LUT Format", str(clf_name), raw_config
    )
...
def generate_clf_common(directory):
    """
    Generate the *ACES* specific *Common LUT Format* (CLF) transforms.
    ...
    _dummy_metadata["ID"] = (
        "urn:aswf:ocio:transformId:v1.0:" "Common.OCIO.Linear_to_Rec1886.c1.v1"
    )
